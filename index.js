...
program
  ...
  .option('-id, --id <id>', 'Locate a document by _id attribute');
...
program
  ...
  program
    .command('put [path]')
    .description('Locates a document by id attribute and inserts it')
    .action((file) => {
        fs.stat(file, (err, stats) => {
            if (err) {
                if (program.json) {
            ...
            if (!program.index || !program.id) {
                const msg = 'No index or id specified! Use --index <name>';
                if (!program.json) throw Error(msg);
                console.log(JSON.stringify({ error: msg }));
                return;
            }
